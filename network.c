...
static int wait_cancellable(struct iio_network_io_context *io_ctx, short events)
{
	struct pollfd pfd[2];
	int ret;
	...
	void *data, size_t len, int flags)
{
	...
	while (1) {
		ret = wait_cancellable(io_ctx, POLLIN);
		if (ret < 0)
			return ret;
	...
	const void *data, size_t len, int flags)
{
	...
	while (1) {
		ret = wait_cancellable(io_ctx, POLLOUT);
		if (ret < 0)
			return ret;
